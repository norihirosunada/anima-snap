...
const GENERIC_OPTIONS = ['とても当てはまめE, '少し当てはまめE, 'どちらとも言えなぁE, 'あまり当てはまらなぁE];

const DEFAULT_QUESTION_SET: QuestionItem[] = [
  {
    id: 'q1',
    question: 'どのくらぁE�E期間、一緒にぁE��すか�E�E,
    options: ['1年以冁E, '1、E年', '3、E年', '5年以丁E],
  },
  {
    id: 'q2',
    question: 'こ�Eモノへの思い入れ�E�E�E,
    options: ['とても大刁E, 'まあまあ大刁E, '普送E, 'なんとなぁE],
  },
  {
    id: 'q3',
    question: 'こ�Eモノと一緒に過ごす時間�E�E�E,
    options: ['毎日', '週に数囁E, '時、E, 'たまに'],
  },
];
...
async function callWithRetry<T>(func: () => Promise<T>, maxRetries = 5): Promise<T> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.warn(`503エラー、E{waitSeconds.toFixed(1)}秒後にリトライ (${attempt + 1}/${maxRetries})`);
...
Respond ONLY with valid JSON:
{
  ...
  try {
    ...
    const result = jsonText
      ...
      : {
          traits: ['神秘的', '静寁E, '古ぁE���E'],
          speechStyle: '静かで落ち着ぁE��話し方',
          backstory: `${objectName}は長ぁE��間をあなた�Eそ�Eで過ごし、多くの記�Eを蓄えてきた。`,
          nickname: '時�E守り人',
...
function buildMemoryContext(memories: Memory[], albumPhotos: AlbumPhoto[]): string {
  ...
  if (memories.length > 0) {
    ...
    const lines = recent.map((m) => {
      ...
      const typeLabel = m.type === 'awakening' ? '✦ 覚�E' : m.type === 're-encounter' ? '☽ 再企E : '✁E会話';
      return `- [${date}] ${typeLabel}: ${m.content.slice(0, 100)}`;
    });
    parts.push(`過去の思い出・記�E:\n${lines.join('\n')}`);
  }
  ...
  if (photosWithLocation.length > 0) {
    const lines = photosWithLocation.map((p) => {
      ...
      const src = p.source === 'initial' ? '初めての出会い' : p.source === 're-encounter' ? '再企E : '記録';
...
${memoryContext ? `\n${memoryContext}\n\nこれら�E記�E・思い出を�E然に会話に織り交ぜてください。場所の思い出があれ�E、その場所につぁE��言及することがあります。` : ''}
Always respond in Japanese. Stay in character as this object's spirit. Keep responses concise (2-4 sentences).`;
  ...
  const contents = [
    ...
    { role: 'model' as const, parts: [{ text: `はぁE��E{obj.personality.nickname}として、あなたとの対話を大刁E��します。` }] },
    ...history.map((h) => ({
      role: h.role,
      parts: [{ text: h.text }],
    })),
...
- Reflects your personality: ${obj.personality.traits.join(', ')}`;
  ...
  return response.text ?? `また会えた�E、E{obj.personality.nickname}はあなた�Eことを覚えてぁE��よ。`;
}
...
export async function generateAwakeningVideo(
  ...
  const prompt = '顔がモノに現れて目覚める瞬間をアニメ皁E��表現してください';

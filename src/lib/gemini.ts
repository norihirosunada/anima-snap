...
const GENERIC_OPTIONS = ['縺ｨ縺ｦ繧ょｽ薙※縺ｯ縺ｾ繧・, '蟆代＠蠖薙※縺ｯ縺ｾ繧・, '縺ｩ縺｡繧峨→繧りｨ縺医↑縺・, '縺ゅ∪繧雁ｽ薙※縺ｯ縺ｾ繧峨↑縺・];

const DEFAULT_QUESTION_SET: QuestionItem[] = [
  {
    id: 'q1',
    question: '縺ｩ縺ｮ縺上ｉ縺・・譛滄俣縲∽ｸ邱偵↓縺・∪縺吶°・・,
    options: ['1蟷ｴ莉･蜀・, '1縲・蟷ｴ', '3縲・蟷ｴ', '5蟷ｴ莉･荳・],
  },
  {
    id: 'q2',
    question: '縺薙・繝｢繝弱∈縺ｮ諤昴＞蜈･繧後・・・,
    options: ['縺ｨ縺ｦ繧ょ､ｧ蛻・, '縺ｾ縺ゅ∪縺ょ､ｧ蛻・, '譎ｮ騾・, '縺ｪ繧薙→縺ｪ縺・],
  },
  {
    id: 'q3',
    question: '縺薙・繝｢繝弱→荳邱偵↓驕弱＃縺呎凾髢薙・・・,
    options: ['豈取律', '騾ｱ縺ｫ謨ｰ蝗・, '譎ゅ・, '縺溘∪縺ｫ'],
  },
];
...
async function callWithRetry<T>(func: () => Promise<T>, maxRetries = 5): Promise<T> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.warn(`503繧ｨ繝ｩ繝ｼ縲・{waitSeconds.toFixed(1)}遘貞ｾ後↓繝ｪ繝医Λ繧､ (${attempt + 1}/${maxRetries})`);
      await sleep(waitSeconds * 1000);
    }
  }
  throw new Error('譛螟ｧ繝ｪ繝医Λ繧､蝗樊焚縺ｫ驕斐＠縺ｾ縺励◆');
}
...
Respond ONLY with valid JSON:
{
  ...
  "nickname": "a cool Japanese nickname/title for this spirit (e.g., '闥ｼ縺肴ｰｴ縺ｮ螳医ｊ莠ｺ')",
  ...
  try {
    ...
    const result = jsonText
      ...
      : {
          traits: ['逾樒ｧ倡噪', '髱吝ｯ・, '蜿､縺・ｨ俶・'],
          speechStyle: '髱吶°縺ｧ關ｽ縺｡逹縺・◆隧ｱ縺玲婿',
          backstory: `${objectName}縺ｯ髟ｷ縺・凾髢薙ｒ縺ゅ↑縺溘・縺昴・縺ｧ驕弱＃縺励∝､壹￥縺ｮ險俶・繧定塘縺医※縺阪◆縲Ａ,
          nickname: '譎ゅ・螳医ｊ莠ｺ',
...
function buildMemoryContext(memories: Memory[], albumPhotos: AlbumPhoto[]): string {
  ...
  if (photosWithLocation.length > 0) {
    const lines = photosWithLocation.map((p) => {
      ...
      const src = p.source === 'initial' ? '蛻昴ａ縺ｦ縺ｮ蜃ｺ莨壹＞' : p.source === 're-encounter' ? '蜀堺ｼ・ : '險倬鹸';
      const place = p.location!.placeName ?? `邱ｯ蠎ｦ${p.location!.latitude.toFixed(4)}, 邨悟ｺｦ${p.location!.longitude.toFixed(4)}`;
      return `- [${date}] ${src} @ ${place}`;
    });
    parts.push(`謦ｮ蠖ｱ蝣ｴ謇縺ｮ險倬鹸:\n${lines.join('\n')}`);
...
${memoryContext ? `\n${memoryContext}\n\n縺薙ｌ繧峨・險俶・繝ｻ諤昴＞蜃ｺ繧定・辟ｶ縺ｫ莨夊ｩｱ縺ｫ郢斐ｊ莠､縺懊※縺上□縺輔＞縲ょｴ謇縺ｮ諤昴＞蜃ｺ縺後≠繧後・縲√◎縺ｮ蝣ｴ謇縺ｫ縺､縺・※險蜿翫☆繧九％縺ｨ縺後≠繧翫∪縺吶Ａ : ''}
Always respond in Japanese. Stay in character as this object's spirit. Keep responses concise (2-4 sentences).`;
  ...
  const contents = [
    ...
    { role: 'model' as const, parts: [{ text: `縺ｯ縺・・{obj.personality.nickname}縺ｨ縺励※縲√≠縺ｪ縺溘→縺ｮ蟇ｾ隧ｱ繧貞､ｧ蛻・↓縺励∪縺吶Ａ }] },
    ...history.map((h) => ({
      role: h.role,
      parts: [{ text: h.text }],
    })),
...
- Reflects your personality: ${obj.personality.traits.join(', ')}`;
  ...
  return response.text ?? `縺ｾ縺滉ｼ壹∴縺溘・縲・{obj.personality.nickname}縺ｯ縺ゅ↑縺溘・縺薙→繧定ｦ壹∴縺ｦ縺・ｋ繧医Ａ;
}
...
export async function generateAwakeningVideo(
  ...
  const prompt = '鬘斐′繝｢繝弱↓迴ｾ繧後※逶ｮ隕壹ａ繧狗椪髢薙ｒ繧｢繝九Γ逧・↓陦ｨ迴ｾ縺励※縺上□縺輔＞';
